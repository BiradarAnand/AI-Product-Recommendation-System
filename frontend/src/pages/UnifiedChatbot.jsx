...
const INIT_OCCASIONS = [
  ...
  { key: "sports",         label: "Sports & Gym",           icon: "🏋️" },
  ...
  { key: "party",          label: "Party & Night Out",      icon: "🎉" },
];
...
// ── Keyword → occasion label map ──────────────────────────────────────────
const KEYWORD_OCCASION_MAP = [
  {
    occasion: "Sports & Gym",
    keywords: [
      "gym", "workout", "fitness", "exercise", "training", "athletic",
      "running", "jogging", "sport", "sports", "cardio", "weights",
      "bodybuilding", "yoga", "crossfit", "cricket", "football", "badminton",
    ],
  },
  {
    occasion: "Wedding / Function",
    keywords: [
      "wedding", "marriage", "shaadi", "nikah", "reception", "engagement",
      "bride", "groom", "function", "ceremony", "mehendi", "sangeet",
      "baraat", "shadi", "barat", "sagai",
    ],
  },
  {
    occasion: "Job Interview",
    keywords: [
      "interview", "job interview", "campus", "placement", "hiring",
      "recruitment", "fresher", "first job",
    ],
  },
  {
    occasion: "Office & Work",
    keywords: [
      "office", "work", "corporate", "professional", "business",
      "meeting", "formal", "boardroom", "client", "presentation",
    ],
  },
  {
    occasion: "Casual Outing",
    keywords: [
      "casual", "outing", "hangout", "weekend", "friends", "mall",
      "chill", "relax", "everyday", "streets", "street", "roam",
    ],
  },
  {
    occasion: "Date Night",
    keywords: [
      "date", "dinner", "night out", "romantic", "evening", "restaurant",
      "anniversary", "valentines", "propose", "girlfriend", "boyfriend",
    ],
  },
  {
    occasion: "Festival & Traditional",
    keywords: [
      "festival", "diwali", "holi", "eid", "puja", "celebrate",
      "traditional", "cultural", "navratri", "dussehra", "ganesh",
      "christmas", "onam", "baisakhi", "lohri",
    ],
  },
  {
    occasion: "Beach & Vacation",
    keywords: [
      "beach", "vacation", "travel", "holiday", "trip", "sea", "pool",
      "resort", "goa", "summer", "tour", "trek", "hiking", "camping",
    ],
  },
  {
    occasion: "Party & Night Out",
    keywords: [
      "party", "club", "night", "dance", "concert", "birthday",
      "celebration", "pub", "bar", "rave", "dj", "nightclub",
    ],
  },
];

// If message contains an occasion keyword, enrich it with the occasion label
// so the backend NLP always resolves correctly.
// "gym outfit" → "Sports & Gym outfit"
// "marriage next week" → "Wedding / Function next week"
function enrichWithOccasion(text) {
  const lower = text.toLowerCase();
  for (const { occasion, keywords } of KEYWORD_OCCASION_MAP) {
    for (const kw of keywords) {
      if (lower.includes(kw)) {
        const regex = new RegExp(kw, "i");
        const enriched = text.replace(regex, occasion);
        return enriched === text ? `${occasion} — ${text}` : enriched;
      }
    }
  }
  return null;
...
export default function UnifiedChatbot() {
  ...
  const send = async (text, extraRefinements = {}) => {
    ...
    // ── Occasion keyword enrichment ──
    // Check if the user typed a keyword like "gym", "wedding", "party" etc.
    // If matched, enrich the message with the occasion label so the backend
    // NLP resolves it correctly, but show the original text in the chat UI.
    const enriched = enrichWithOccasion(raw);
    const msg = enriched || raw;

    push({ role: "user", type: "text", text: raw }); // show original in UI
    setLoading(true);
    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[390px] max-w-[calc(100vw-24px)] h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
          ...
          {/* Occasion quick-pick tabs */}
          <div className="px-3 pt-2 pb-1 flex gap-1.5 overflow-x-auto flex-shrink-0 border-b border-gray-100"
            style={{ scrollbarWidth: "none" }}>
            <style>{`.occ-tabs::-webkit-scrollbar{display:none}`}</style>
            {INIT_OCCASIONS.map(occ => (
              <button key={occ.key}
                onClick={() => send(occ.label)}
                disabled={loading}
                className="flex-shrink-0 flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full border border-gray-200 text-gray-600 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all disabled:opacity-40 whitespace-nowrap font-medium">
                <span>{occ.icon}</span>
                <span>{occ.label}</span>
              </button>
            ))}
          ...
          {/* Budget filter pills */}
          <div className="px-3 pt-1.5 pb-1.5 flex gap-2 overflow-x-auto flex-shrink-0 border-b border-gray-100">
            <span className="flex-shrink-0 text-xs text-gray-400 font-semibold self-center">Budget:</span>
            {[
              { key: "",        label: "All" },
          ...
          <div className="px-3 py-3 border-t border-gray-100 bg-white flex-shrink-0">
            <div className="flex gap-2 items-end">
              <textarea
                ...
                placeholder='Try "gym", "marriage", "beach trip"…'
                rows={1}
                disabled={loading}
